...
ROUTING:visitor-pages=/dear/[company](on-demand-ISR,revalidate=3600,NO-generateStaticParams,notFound-on-unknown,revalidatePath-via-Visitors-afterChange;Visitors-collection=data-source)+landing=ISR(revalidate=3600);portfolio-details=app-router-per-route+shared-section-layout(persistent-sidebar,features/menu/PortfolioNav.tsx)+soft-nav(NOT-bespoke-SPA);detail-data=server-fetched-via-Local-API
...
KEYWORDS:single-collection=SSOT(Keywords:label,slug);two-axis(category:scope|craft + searchOnly:bool)+aliases[](feed-Fuse,never-render)+target(searchOnly-only:kind=route→relationship | kind=section→href e.g./#id);content-attaches-via-TWO-filterOptions-scoped-relationship-fields(scope,craft)NOT-single-field,NOT-separate-collections;display-order=relationship-attach-order(drag-reorder,NO-render-time-sort);searchOnly-keywords-NOT-in-scope/craft-pickers(alias→target→SearchDocument.href);fragment-href(/#id)=link-from-inner-page + scroll-on-landing(needs scroll-margin-top on section-anchors)
