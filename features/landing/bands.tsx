...
import { proseLines, type LandingCardData } from './projections'
  ...
  return (
    <section id={id} className={`${BAND} scroll-mt-24`}>
      ...
      <div className="mt-10 rounded-2xl border border-dashed border-border p-6 sm:p-10">
        {teaser?.eyebrow && (
          <p className="text-xs font-semibold uppercase tracking-widest text-primary">{teaser.eyebrow}</p>
        )}
        {teaser?.title && (
          <h3 className="mt-1 text-xl font-semibold tracking-tight">{teaser.title}</h3>
        )}
        {teaser?.description && <p className="mt-3 text-muted-foreground">{teaser.description}</p>}
